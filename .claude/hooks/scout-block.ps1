...
if ($toolInput.command -and ($toolInput.command -is [string])) {
    # Allow wails build commands (wails build, wails build -clean, etc.)
    $isWailsBuild = $toolInput.command -match '(^|\s|&&|\|)wails\s+build'
    if (-not $isWailsBuild -and ($toolInput.command -match $blockedBashPattern)) {
        Write-Error "ERROR: Blocked directory pattern (node_modules, __pycache__, .git/, dist/, build/)"
        exit 2
    }
}
